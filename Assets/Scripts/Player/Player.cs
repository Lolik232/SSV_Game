...
public class Player : MonoBehaviour
{
	...
	[NonSerialized] public bool isHanging;
	[NonSerialized] public bool isClimbingLedge;
	...
	public void DetermineCornerPosition()
	{
		foreach (var target in _whatIsTarget)
		{
			...
			if (hit)
			{
				cornerPosition.Set(wallPosition.x, _ledgeChecker.position.y - hit.distance);
				ledgeStartPosition.Set(cornerPosition.x + wallDirection * (_startLedgeOffset.x + 0.02f),
															 cornerPosition.y - _startLedgeOffset.y - 0.02f);
				ledgeEndPosition.Set(cornerPosition.x - wallDirection * _endLedgeOffset.x,
														 cornerPosition.y + _endLedgeOffset.y + 0.02f);
				return;
			}
		}
	}
	...
	public void DoChecks()
	{
		...
		if (isTouchingWall && !isTouchingLedge && !isHanging && !isClimbingLedge)
		{
